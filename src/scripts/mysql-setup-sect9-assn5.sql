...
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, REFERENCES, INDEX, ALTER, EXECUTE, CREATE VIEW, SHOW VIEW,
    CREATE ROUTINE, ALTER ROUTINE, EVENT, TRIGGER ON `jt_hibernate_jpa_bookdb2`.* TO `bookadmin`@`%`;
CREATE USER IF NOT EXISTS `bookuser`@`%` IDENTIFIED WITH mysql_native_password BY 'bookuser';
GRANT SELECT, INSERT, UPDATE, DELETE, SHOW VIEW ON `jt_hibernate_jpa_bookdb2`.* TO `bookuser`@`%`;
FLUSH PRIVILEGES;